...
class OpusFlutterWindows extends OpusFlutterPlatform {
  ...
  static Future<String> _copyFiles() async {
    ...
    Directory dir =
        Directory('$tmpPath/opus_flutter_new_windows/opus').absolute;
    ...
    if (!(await f.exists())) {
      data = await rootBundle
          .load('packages/opus_flutter_new_windows/assets/$_licenseFile');
    ...
    if (!(await f.exists())) {
      data = await rootBundle
          .load('packages/opus_flutter_new_windows/assets/$src');
      await f.writeAsBytes(
          data.buffer.asUint8List(data.offsetInBytes, data.lengthInBytes));
    }
    return f.path;
